document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveCurrentDraft() { const data = getFormDataAsObject(); localStorage.setItem('cvDraft', JSON.stringify(data)); }
    function loadCurrentDraft() { const draft = localStorage.getItem('cvDraft'); if (draft) { populateFormFromObject(JSON.parse(draft)); } else { startNewCv(false); } }
    ...
    function updateCvPreview() {
        ...
        const skillsHtml = (data.skills || '').split(',')
            .map(s => s.trim())
            .filter(s => s)
            .map(skill => `<div class="skill-item"><i class="${getSkillIcon(skill)}"></i><span>${skill}</span></div>`)
            .join('');
        ...
        previewContainer.innerHTML = `
            ...
            <footer class="cv-final-note">
                <p class="m-0">Si quieres saber más información de mí, contacta una cita y te muestro mis habilidades, proyectos y qué tan valioso puedo ser para ti.</p>
            </footer>`;
    }
    ...
    function saveCvToHistory() { const cvData = getFormDataAsObject(); if (!cvData.name.trim()) { alert('Por favor, ingresa al menos un nombre para guardar el CV.'); return; } let history = JSON.parse(localStorage.getItem('cvHistory')) || []; const existingIndex = history.findIndex(cv => cv.id == cvData.id); if (existingIndex > -1) { history[existingIndex] = cvData; } else { history.push(cvData); } localStorage.setItem('cvHistory', JSON.stringify(history)); alert('¡CV guardado en tu historial!'); renderHistory(); }
    function renderHistory() { const history = JSON.parse(localStorage.getItem('cvHistory')) || []; historyListContainer.innerHTML = history.length === 0 ? '<div class="col-12"><p class="text-center text-muted">Aún no has guardado ningún CV.</p></div>' : history.map(cv => `<div class="col-md-4"><div class="history-card"><h5>${cv.name}</h5><p class="text-muted mb-2">${cv.title || 'Sin título'}</p><button class="btn btn-sm btn-primary edit-cv-btn" data-id="${cv.id}">Editar</button> <button class="btn btn-sm btn-danger delete-cv-btn" data-id="${cv.id}">Eliminar</button></div></div>`).join(''); }
    historyListContainer.addEventListener('click', e => { const id = e.target.dataset.id; if (!id) return; if (e.target.classList.contains('edit-cv-btn')) { let history = JSON.parse(localStorage.getItem('cvHistory')); const cvToLoad = history.find(cv => cv.id == id); if (cvToLoad) { populateFormFromObject(cvToLoad); alert('CV cargado en el editor. ¡Puedes modificarlo y volver a guardarlo o descargarlo!'); window.scrollTo({ top: 0, behavior: 'smooth' }); } } if (e.target.classList.contains('delete-cv-btn')) { if (confirm('¿Estás seguro de que quieres eliminar este CV?')) { let history = JSON.parse(localStorage.getItem('cvHistory')); history = history.filter(cv => cv.id != id); localStorage.setItem('cvHistory', JSON.stringify(history)); renderHistory(); } } });
    function startNewCv(confirmFirst = true) { const doReset = () => { cvForm.reset(); document.getElementById('cv-id').value = ''; ['experience', 'education', 'extra', 'reference'].forEach(type => { document.getElementById(`${type}-entries`).innerHTML = ''; window[`${type}Count`] = 0; }); localStorage.removeItem('cvDraft'); addExperienceEntry(); addEducationEntry(); addReferenceEntry(); updateCvPreview(); }; if (confirmFirst) { if (confirm('¿Estás seguro? Se borrarán los datos no guardados del formulario actual.')) { doReset(); } } else { doReset(); } }

    // --- EXPORTACIÓN A PDF Y FUNCIONES AUXILIARES ---
    async function exportToPdf() {
        const cvToExport = document.getElementById('cv-preview');
        if (!document.getElementById('cv-name').value.trim()) {
        ...
        const originalPosition = viewerContainer.style.position;
        viewerContainer.style.position = 'relative'; // Crucial para html2canvas

        try {
            const canvas = await html2canvas(cvToExport, {
                scale: 3, // Mayor escala para mejor calidad de impresión
                useCORS: true,
                // No necesitamos scrollY: -window.scrollY aquí, ya que el elemento cv-preview
                // crece a su altura total y html2canvas lo captura completo.
            });
            ...
            let pdfFormat = 'letter'; // Formato inicial: 8.5 x 11 pulgadas
            const LETTER_WIDTH_IN = 8.5;
            const LETTER_HEIGHT_IN = 11;
            const LEGAL_HEIGHT_IN = 14; // Formato legal: 8.5 x 14 pulgadas

            // Calcular la altura que el contenido tomaría si su ancho se ajusta a una página LETTER
            const scaledHeightForLetterWidth = (imgHeightPx * LETTER_WIDTH_IN) / imgWidthPx;

            // Si la altura escalada excede el alto de una página Carta, cambiamos a Legal
            if (scaledHeightForLetterWidth > LETTER_HEIGHT_IN) {
                pdfFormat = 'legal';
                alert("El contenido es extenso. Se generará un PDF en formato Oficio/Legal (8.5x14) para que todo quepa en una sola página.");
            }

            const pdf = new jsPDF({
                orientation: 'portrait',
                unit: 'in', // Trabajar en pulgadas para mayor precisión
                format: pdfFormat
            });
            ...
            // Calcular las dimensiones finales de la imagen para que encaje *exactamente*
            // dentro de la página PDF, manteniendo su relación de aspecto.
            let finalImgWidthIn, finalImgHeightIn;
            const aspectRatio = imgWidthPx / imgHeightPx;

            // Opción 1: Ajustar por el ancho de la página
            finalImgWidthIn = pdfPageWidth;
            finalImgHeightIn = finalImgWidthIn / aspectRatio;

            // Si al ajustar por ancho, la imagen es más alta que la página, entonces ajustamos por alto
            if (finalImgHeightIn > pdfPageHeight) {
                finalImgHeightIn = pdfPageHeight;
                finalImgWidthIn = finalImgHeightIn * aspectRatio;
            }

            // Centrar la imagen en la página (si no ocupa todo el espacio)
            const xOffset = (pdfPageWidth - finalImgWidthIn) / 2;
            const yOffset = (pdfPageHeight - finalImgHeightIn) / 2;

            pdf.addImage(imgData, 'PNG', xOffset, yOffset, finalImgWidthIn, finalImgHeightIn);
            pdf.save(`${document.getElementById('cv-name').value.replace(/\s+/g, '_')}_CV.pdf`);

        } catch (error) {
            console.error('Error al generar el PDF:', error);
            alert('Ocurrió un error al generar el PDF. Por favor, intenta de nuevo.');
        } finally {
            viewerContainer.style.position = originalPosition; // Restaurar el estilo original
        }
    }
